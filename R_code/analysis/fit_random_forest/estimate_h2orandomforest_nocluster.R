...
# Construct case weights variable and drop unused factor levels — mirrors ranger version
estimation_dataset <- estimation_dataset %>%
  mutate(
    weighting   = frequency_weights(weighting),
...
validation_data <- validation(data_split)  # the "calibrate" split
...
# last_fit() fits on train + validation combined and evaluates on test.
...
final_workflow_bundled <- bundle(final_workflow)
...
train_preds <- augment(final_fitted_wf, new_data = train_data)
val_preds   <- augment(final_fitted_wf, new_data = validation_data)
test_preds  <- augment(final_fitted_wf, new_data = test_data)
