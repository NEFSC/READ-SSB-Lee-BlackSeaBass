######  
#  **********************************************************************
#  * Purpose: 	Do some ESDA on my validation data to learn why the "South" isn't doing so great.
#  * Inputs:
#    - final_fit
#    - initial data_split    
#
...
vintage_string <- "2025-08-19"  
final_fit <- read_rds(here("results", "ranger", glue("BSB_ranger_South_region_nocluster_results{vintage_string}.Rds")))
data_split <- read_rds(here("results", "ranger",glue("nocluster_South_region_data_split{vintage_string}.Rds")))
#############################################################################
# END  Data read in #
#############################################################################
...
datacheck2<-test_predictions %>%
  mutate(investigate=case_when(
    ...
    .pred_class == "Jumbo" & market_desc == "Large" ~ "LasJ",
    .pred_class == "Jumbo" & market_desc == "Jumbo" ~ "CJASJ",
    .pred_class == "Small" & market_desc == "Small" ~ "CSASS",
...
datacheck2<-datacheck2 %>%
  mutate(investigate=fct_relevel(investigate,c("CJASJ", "JasL","JasM","LasJ","CLASL","LasM", "CMASM", "CSASS","OtherCorrect", "OtherWrong","Unknown")) ) 



datacheck2 %>%
  DataExplorer::create_report(
    output_file = here("R_code","analysis","esda",glue("SouthR_ESDA_by_misclassifcation_{DateTime}")),
    report_title = "EDA Report - South EDA by mis-classification",
    y = "investigate",
    config=configs
  )


# looking at the box plot of priceR by classification status, it looks like the misclassifieds have the "wrong" price

ggplot(datacheck2, aes(y=priceR_CPI, x=investigate)) + 
  geom_boxplot()
...
ggplot(datacheck2 %>% filter(!investigate %in% c("CJASJ","CSASS", "CMASM")), aes(y=.pred_Large, x=priceR_CPI, color=investigate)) + 
  geom_point()

# this is an interesting graph. The's a range of lower prices for Large, where we're predicting Larges as Mediums. 
# And a range of higher
ggplot(datacheck2 %>% filter(investigate %in% c("CJASJ","CLASL", "LasJ", "LasM")), aes(y=.pred_Large, x=priceR_CPI, color=investigate)) + 
  geom_point()


ggplot(datacheck2 %>% filter(investigate %in% c("LasJ", "LasM")), aes(y=.pred_Jumbo, x=.pred_Large, color=priceR_CPI, shape=state)) + 
  geom_point(aes(size=1)) + 
  scale_color_gradient(low = "lightblue", high = "darkred")

ggplot(datacheck2 %>% filter(investigate %in% c("LasJ", "LasM")), aes(y=.pred_Jumbo, x=.pred_Large, color=state)) + 
  geom_point(aes(size=1)) + 
...
# unweighted
ggplot(datacheck2, aes(y=priceR_CPI, fill=market_desc, x=state)) + 
  geom_boxplot()
# weighted by landed pounds
# The box plots for the validation and training datasets look similar.
ggplot(datacheck2 %>% filter(year %in% c(2021,2022,2023,2024)), aes(y=priceR_CPI, fill=market_desc, x=state, weight=lndlb)) + 
  geom_boxplot() + ggtitle("weighted prices, validation dataset")

ggplot(train_data  %>% filter(year %in% c(2021,2022,2023,2024)), aes(y=priceR_CPI, fill=market_desc, x=state, weight=lndlb)) + 
  geom_boxplot() + ggtitle("weighted prices, training dataset")

#facet wrapped on year


ggplot(datacheck2, aes(y=priceR_CPI, fill=market_desc, x=state, weight=lndlb)) + 
  geom_boxplot() +
  facet_wrap(vars(year))  + ggtitle("weighted prices, validation dataset")


ggplot(train_data, aes(y=priceR_CPI, fill=market_desc, x=state, weight=lndlb)) + 
  geom_boxplot() +
  facet_wrap(vars(year)) + ggtitle("weighted prices, training dataset")
...
# Boxplot of training data -- grouped a different way
ggplot(train_data %>% filter(year==2019 ), aes(y=priceR_CPI, fill=state, x=market_desc, weight=lndlb)) + 
  geom_boxplot() 
...
datacheck2 %>%
  DataExplorer::create_report(
    output_file = here("R_code","analysis","esda",glue("SouthR_ESDA_by_state_{DateTime}")),
    report_title = "EDA Report - South EDA by mis-classification",
    y = "state",
    config=configs
  )
