...
Our ultimate goal is to predict the numbers at length
for the un-measured "Unclassified" fish at the relevant scale for the stock
assessment. Rather, we predict the probabilities of class membership, that is "Small", 
...
a prediction about the Unclassified fish. Because our goal is a good aggregate
prediction, in this case at the year-stock unit-semester level, the accuracy of
an individual prediction is of limited interest.
...
There is quite a bit of fishing that occurs within state waters, so the Vessel
...
grade fish into particular classes. In our preferred specification, we
group by "dealer identifier" so that all of a dealer's observations are contained
...
We use the mean log-loss function to do model tuning.  After tuning, we train 
...
Including a full set of binary indicator variables to account dealer-specific 
