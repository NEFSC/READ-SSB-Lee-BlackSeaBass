...
these unmeasured fish. The Black Sea Bass (*Centropristis striata*) fishery in the Northeast United States
...
Dealers sort landings into 4 primary market categories: Jumbo, Large, Medium, and Small. 
There is also an Unclassified or Mixed category.  From 2020 to 2023, 5 to 10% of
commercial landings were in the “Unclassified” market category; however no fish 
in this category were measured. The stock assesment used some data to convert 
the weights of these fish into an age structure.

The general approach is as follows (terrible writing).  As part of our exploratory data analysis, we estimate a simple hedonic model, and verify that prices vary by market category. We  train a random forest model to classify fish into one of the four primary market categories.
Finally, we conduct out-of-sample prediction on fish that are reported as Unclassified.
...
predictions of the Unclassified fish to be aggregated to the year-stock unit level andt the accuracy of
an individual prediction is of no interest. 
...
the spring and offshore movement affects things.  When fish are offshore, they are
...
```{r length-histograms,echo=FALSE,out.width="44%", fig.show='hold', fig.align='center',fig.cap="Length and Price Frequencies by market categories."}
...
We use CAMS data from 2015 to 2024 to train our data. We set aside 10% of our
data as a hold out sample to validate model fit.  

We divide the remaining 90\% of the data into 10 approximately equal folds, again
