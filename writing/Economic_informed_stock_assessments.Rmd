...
In this application, we use a multinomial random forest model [@Breiman2001; @Malley2012] to model the probability that a transaction is in one of the four principle market categories^[Random forest pplications of these models in fisheries include classification of acoustic data [@Fallon2016, @Proud2020], fishery metiers [@Oostdijk2024], and habitat [@Liu2025].].  Random Forests are can viewed as ensembles of Classification and Regression Trees [CART, @Breiman1984].  A classification tree repeatedly partitions observations into two groups based on a single explanatory variable.  The optimal explanatory variable to use and value at which the split occurs is selected by minimizing a measure of variation among member of the partition; we use gini impurity.  Intuitively, the best split produces groups with homogenous outcomes.  The CART algorithm grows tress by continuing to split until all nodes are homogenous or a user-specified early stopping criteria is met. 

Single classification trees are prone to poor out of sample performance; a tree grown to pure nodes is perfectly fit to the training dataset.  Random forests improve out of sample prediction by bootstrapping and restricting each bootstrap replication to a small number of randomly selected predictors [@Breiman2001; @Strobl2009, @Hastie2023 ].  Both modifications induce variation in the individual classification trees and leads to better out-of-sample performance. @Malley2012 and @Kruppa2014 use the final nodes of random forests to assign probability of class membership Random Forests, we adopt this approach. 

Like many machine learning models, random forests have hyperparameters which must be exogenously supplied. Results can be sensitive to these choices and a common approach is to tune the model by searching for an optimal set of parameters using a subset of data and validate model performance by using the preferred model to predict class membership on a holdout sample.  Random forest models are also known to produce incorrect predicted probabilities. This occurs because the model averaging process in which individual trees are grown into random forests also averages away. stuff.  Calibration is often necessary after fitting a random forest [@Niculescu-Mizil2005, @Kull2017; @Ojeda2023; @Barreada2024; @Ferrer2025] and an auxiliary regression to calibrate these probabilities is often required.  

To address these blah, we do a bunch of good stuff.  We split our dataset three ways; we use 70% of our dataset to tune the model hyperparameters, 15% to calibrate probabilities, and 15% to validate the model.  Simulation studies by [@Barreada2024] suggest tuning over the minimum node size (*min.n*) when using random forests in probability models. The number of randomly selected predictors (*mtry*) was also tuned over. We tune over two hyperparameters by performing a 10-fold cross validation study and performing a grid search over the reasonable starting values of the two parameters.  We supplemented the initial grid search by performing the bayesian tuning described by @Shahriari2016. Our cross-validation was stratified by market category to ensure that the proportion of each class in the folds approximately matched the initial compositions. We selected the optimal pair of hyperparameters that had the smallest log loss criterion function averaged over the ten folds. For each fold, log loss is computed as: 
...
After selecting the best hyperparamters, we refit the model on the entire 70% of the dataset used to select hyperparameters.  We use the fitted model to predict class membership for the 15% calibration sample and visually assess whether the model is well-calibrated using commonly used calibration plots [@Ojeda2023; @Barreada2024]. We use the Relative Calibration loss methods described in [@Ferrer2025] to assess whether calibration is necessary (it is) and to select among isotonic, beta, and multinomial calibration methods.
...
The core concept of this research is simple: we can use the information contained in transaction level prices to infer something about the size of the Unclassified fish.  A simple theoretical model sketch can provide guidance about explanatory factors that belong in the model. We assume a perfectly competitive market for *ex-vessel* fish: there are many dealers and vessels, no vertical integration,  no transactions costs associated with selling or storing fish, all dealers and vessels know the prices being paid and the quantities begin supplied, all attributes of all trades are observed by everyone, and individuals are small relative to the market and cannot influence market prices. <!--- We also assume there is no vertical integration between dealers and vessels: a dealer and a fishing vessel is not owned by the same firm and there is no "forward" contracting in which buyers and sellers agree in advance on a price. --->

To build intuition, we assume there are three categories of fish: Jumbo, Large, and Unclassified. Prices of fish are constant and dealers strictly prefer Jumbo to Large, so the price of Jumbo is strictly greater than the price of Large: $p^J > p^L$.  Unclassified transactions are composed of $\alpha$ Jumbo and $(1-\alpha)$ Large. Because the buyers and sellers can observe this mix, the price of an unclassified transaction is:

\begin{equation}
\label{unknown1}
p= \alpha p^J+ (1-\alpha) p^L \hspace{2mm} \forall \hspace{2mm} \alpha \in [0,1]
\end{equation},
...
Constant prices for each category is unrealistic for many reasons. In this application, sales transactions are of similar, but not identical goods.  The hedonic model (Table \ref{HedonicTableA}) suggests that many attributes, including size, method of production, and time of year impact prices.  Transactions on different days are different in two dimensions: seasonal variation in meat quality and daily variation in quantity supplied can both impact prices. Regional differences in transportation costs, processing costs,  or final consumer preferences and incomes can result in regional variation in prices [@samuelson1954transfer; @Krugman1991a].  The demand curve for nearly fish is downward sloping: increases in quantity supplied of a category of fish will will reduce the price of that category [@Lee2014; @Ardini2018, and Table \ref{HedonicTableA}]. The different market categories are substitutes: increases in the quantity supplied of Large will reduce the price of Jumbo and vise-versa. 

Because many attributes impact prices, transactions of different size classes may have similar prices.  This suggests that attributes that are important in determining the price of fish should enter as factors in the machine learning model. Intuition for this can be seen in the following example based on the Table \ref{HedonicTableA}. Most fish are delivered "Round," dead with the gut in.  Some fish are delivered live, these receive a premium of approximately \$0.16 per pound. The mean price of a Small Live black sea bass is \$3.69 (=\$3.526 +\$0.164), which is very close to the mean price of Medium Round \$3.75.  Without including an indicator for Live, a machine learning model would be likely to classify "Small Live" black sea bass as "Medium."  We collect all the factors that impact prices into $Z$.    
...
Downward sloping demand implies that equation \ref{unknown1} should be modified by substituting in \ref{demand}.
...
The assumption of a competitive market is critical to our ability to use prices to infer market category. In brief, assume that a dealer did not offer the prevailing Jumbo price to a fishing vessel, instead offering a lower price. A competitive market ensures that this dealer would not be able to purchase any Jumbo fish at this price; vessels with Jumbo fish to sell would simply sell to another dealer.  If the dealer wanted to purchase Jumbos, it would have to raise the offer price.  All identical goods will sell at the same price; observations of identical goods with different prices suggest that some conditions of a competitive market are absent [@Lamont2003].  Variations of the law of one price that account for heterogenous goods, informational assymmetries [@Salop1982], and different data availability have been proposed and tested statistically [@Asche1999; @Nielsen2009; @Pettersen2018]. 
