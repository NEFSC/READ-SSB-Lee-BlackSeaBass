...
} else if (modeltype=="h2onocluster"){
  data_pattern<-"h2o_nocluster_data_split"
  tuning_pattern<-"BSB_h2o_nocluster_tune"
  final_pattern<-"BSB_h2o_nocluster_results"
  split_metrics_pattern<-"BSB_h2o_nocluster_split_metrics"
}  else {
...
data_vintage_string<-list.files(here("results","h2o"), pattern=glob2rx(glue("{data_pattern}*Rds")))
...
tuning_vintage<-list.files(here("results","h2o"), pattern=glob2rx(glue("{tuning_pattern}*Rds")))
...
metrics_vintage<-list.files(here("results","h2o"), pattern=glob2rx(glue("{split_metrics_pattern}*Rds")))
metrics_vintage<-gsub(split_metrics_pattern,"",metrics_vintage)
metrics_vintage<-gsub(".Rds","",metrics_vintage)
metrics_vintage<-max(metrics_vintage)
...
# turn on h2o
h2o.init(
  nthreads     = 16,   # adjust to match your hardware
  max_mem_size = "72g" # adjust to match your hardware
)
...
data_split<-readr::read_rds(file=here("results","h2o",glue("{data_pattern}{data_vintage_string}.Rds")))
...
final_fit_bundled <- read_rds(
  file = here("results", "h2o", glue("{final_pattern}{finalfit_vintage}.Rds"))
)
final_fit <- unbundle(final_fit_bundled)
...
# And the recipe
extract_recipe(final_workflow)

preds<-final_workflow[[1]]$pre$actions$recipe$recipe$var_info %>%
...
split_metrics <- read_rds(
  file = here("results", "h2o", glue("{split_metrics_pattern}{metrics_vintage}.Rds"))
)
...
#   collect_metrics()
split_metrics$test_set_metrics
...
NEED TO LOOK AT THIS closer. I'm not sure if I have permutation importance or gini impurity. I want perm.
```{r VIP}
...
final_pr<-final_predictions %>%
  ...
  autoplot()
ggsave(here("results","h2o","final",glue("pr_curve{modeltype}{finalfit_vintage}.png")), plot=final_pr)
```
...
write_rds(calibrate_iso, file=here("results","h2o","final",glue("calibrate_iso_{modeltype}{finalfit_vintage}.Rds")))
...
ggsave(here("results","h2o","final",glue("cal_multicalibrate_window{modeltype}{finalfit_vintage}.png")), plot=multi_applied_window)



beta_applied_window<-cal_plot_windowed(validation_data_betacalib_applied,
                  truth = market_desc, step_size = 0.05, include_points =FALSE)
ggsave(here("results","h2o","final",glue("cal_betavalidate_window{modeltype}{finalfit_vintage}.png")), plot=beta_applied_window)
...
final_roc<-validation_data_isocalib_applied %>%
  ...
  autoplot()
ggsave(here("results","h2o","final",glue("validation_roc_curve{modeltype}{finalfit_vintage}.png")), plot=final_roc)
...
final_roc_nocal<-validation_data %>%
  roc_curve(truth=market_desc, 
            ...
            case_weights=weighting)  %>%
  autoplot()
ggsave(here("results","h2o","final",glue("validation_uncal_roc_curve{modeltype}{finalfit_vintage}.png")), plot=final_roc_nocal)
...
ggsave(here("results","h2o","final",glue("validation_nocal_roc_curve{modeltype}{finalfit_vintage}.pdf")),
       plot   = p_facet,
...
p_facetUW
ggsave(here("results","h2o","final",glue("validation_nocal_UWroc_curve{modeltype}{finalfit_vintage}.pdf")),
       plot   = p_facetUW,
       width  = 174,   # mm — double-column, suits 4 facets in a row
       height = 65,
       units  = "mm",
...
p_facet
# --- 5. Save at ICES JMS specifications ---
ggsave(here("results","h2o","final",glue("validation_calibrated_roc_curve{modeltype}{finalfit_vintage}.pdf")),
       plot   = p_facet,
       width  = 174,   # mm — double-column, suits 4 facets in a row
       height = 65,
       units  = "mm",
...
p_facetUW
ggsave(here("results","h2o","final",glue("validation_calibrated_UWroc_curve{modeltype}{finalfit_vintage}.pdf")),
       plot   = p_facetUW,
       width  = 174,   # mm — double-column, suits 4 facets in a row
       height = 65,
       units  = "mm",
...
#ggsave(here("results","h2o","final",glue("validation_raw_{modeltype}{finalfit_vintage}.png")), plot=uncalibrated)
#write_rds(uncalibrated, file=here("results","h2o","final",glue("validation_raw_{modeltype}{finalfit_vintage}.Rds")))
...
p_facet
# --- 5. Save at ICES JMS specifications ---
ggsave(here("results","h2o","final",glue("testing_calibrated_roc_curve{modeltype}{finalfit_vintage}.pdf")),
       plot   = p_facet,
       width  = 174,   # mm — double-column, suits 4 facets in a row
       height = 65,
       units  = "mm",
...
p_facetUW
ggsave(here("results","h2o","final",glue("testing_calibrated_UW_ROC_curve{modeltype}{finalfit_vintage}.pdf")),
       plot   = p_facetUW,
       width  = 174,   # mm — double-column, suits 4 facets in a row
       height = 65,
       units  = "mm",
...
write_rds(test_predictions, file=here("results","h2o",glue("aggregate_test_predictions_{modeltype}{finalfit_vintage}.Rds")))
...
write_rds(nocal_predictions, file=here("results","h2o",glue("aggregate_nocal_predictions_{modeltype}{finalfit_vintage}.Rds")))
