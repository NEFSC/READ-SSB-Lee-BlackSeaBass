# Code to make all other figures
...
lfplot.mktcomb.yrbin <- fig.data %>%
  ggplot() +
  geom_histogram(aes(x=LENGTH, y=after_stat(density), fill=yr.bin), alpha=0.6, bins=nbins, colour='black', position='identity') +
...
lfplot.mktcomb.region <- fig.data %>%
  ggplot() +
  geom_histogram(aes(x=LENGTH,y=after_stat(density), fill=REGION), alpha=0.4, bins=nbins, colour='black', position='identity') +
...
lfplot.mktcomb.region <- plot.data %>% 
  ggplot() +
  geom_histogram(aes(x=LENGTH,y=after_stat(density), fill=MKTCOMB), alpha=0.6, bins=nbins, colour='black', position='identity') +
  facet_grid(rows=vars(REGION), cols=NULL, scales = 'free')
print(lfplot.mktcomb.region)
