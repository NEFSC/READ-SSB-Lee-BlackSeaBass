...
2. Three states have implemented a catch share for Black Sea Bass. What has happened in these fisheries?
    ...
    * The quota markets might be thin. Does this mean we have to model quota usage in a dynamic setting? 
  
3. Many of the states have enacted limited access for landing permits. They tend to be the southern states.  What are the welfare effects? Is this a good idea? What is the political economy?  What is the economic game and strategic interactions at the ASFMC and what are the objectives of the individuals?  
    ...
    * Have the possession limits concentrated fishin effort in the nearshore (summer) months? Is this good or bad?  Are offshore northern waters basically inaccessible?
...
New York also shifted to 1/2 day trips in 2009. I suspect a possession limit change did this.  Possession limits make it difficult to go far offshore and shift effort inshore.
...
kbl(stat_area_summary  %>%
  ...
  , format.args = list(big.mark = ","), digits=0,booktabs=T, align=c("l",rep("r",ncol(summ1)-1)), caption =  "Landings (thousands of pounds) over the full time series and recent 5 years, by statistical area") %>%
kable_styling(full_width = T,latex_options = "hold_position") %>% 
      row_spec(0,bold=FALSE) 
...
The following table contains the Total number of active BSB permits and the number of permits that switched ports by year. In 1996, there were 516 active permits; 418 always landed BSB in the same port, 39 switched 1 time, 41 switched twice, 4 switched 3 times, ....

A switch is defined when sequential BSB trips did not land in the same port.
...
port_switchers_yr <-attrib %>%
  ...
  mutate(changed_port=ifelse(changed_port>=10,10,changed_port))


port_switcher_table<-table(port_switchers_yr$year, port_switchers_yr$changed_port)
 
port_switcher_table<-cbind(rowSums(port_switcher_table),port_switcher_table)
colnames(port_switcher_table)[1]<-"Total"


# just show the stat areas greater than 1.5%
kbl(port_switcher_table
  , format.args = list(big.mark = ","), digits=0,booktabs=T, align=c("l",rep("r",ncol(port_switcher_table)-1)), caption =  "Number of permits that switched ports (truncated at 10)") %>%
kable_styling(full_width = T,latex_options = "hold_position") %>% 
      row_spec(0,bold=FALSE) 
...
state_switchers<-attrib %>%
  group_by(permit)%>%
  mutate(state_diff = statecd - dplyr::lag(statecd))%>%
...
state_switchers_yr <-state_switchers %>%
  ...
  mutate(changed_state=ifelse(changed_state>=10,10,changed_state))




state_switcher_table<-table(state_switchers_yr$year, state_switchers_yr$changed_state)
 
state_switcher_table<-cbind(rowSums(state_switcher_table),state_switcher_table)
colnames(state_switcher_table)[1]<-"Total"



# just show the stat areas greater than 1.5%
kbl(state_switcher_table
  , format.args = list(big.mark = ","), digits=0,booktabs=T, align=c("l",rep("r",ncol(port_switcher_table)-1)), caption =  "Number of permits that switched ports (truncated at 10)") %>%
kable_styling(full_width = T,latex_options = "hold_position") %>% 
      row_spec(0,bold=FALSE) 
