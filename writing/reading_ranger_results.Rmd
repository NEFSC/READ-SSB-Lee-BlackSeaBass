...
write_rds(aggregate_predictions, file=here("results","ranger",paste0("aggregate_test_predictions_",modeltype,finalfit_vintage,".Rds")))
...
for (l in c("Jumbo", "Large", "Medium", "Small")) {
  tryCatch({
    full_predictions[[paste0("pred_", l)]] <- test_data[[paste0(".pred_", l)]] * full_predictions$lndlb
  }, error = function(e) {
    # R will just silently continue to the next iteration
  })
}
...
oos_predictions<-predict(final_workflow, oos_data, type="prob")
write_rds(oos_predictions, file=here("results","ranger",paste0("out_of_sample_predictions_",modeltype,finalfit_vintage,".Rds")))

oos_data<-cbind(oos_predictions, oos_data)
...
for (l in c("Jumbo", "Large", "Medium", "Small")) {
  tryCatch({
    oos_data[[paste0("pred_", l)]] <- oos_data[[paste0(".pred_", l)]] * oos_data$lndlb
  }, error = function(e) {
    # R will just silently continue to the next iteration
  })
}
 ...
 aggregate_oos_predictions<-oos_data %>%
   group_by(stockarea, year,market_desc) %>%
  summarise(across(c(starts_with("pred"),"lndlb"), \(x) mean(x, na.rm = TRUE))) %>%
   ungroup()
aggregate_oos_predictions$modeltype<-modeltype
 
write_rds(aggregate_oos_predictions, file=here("results","ranger",paste0("aggregate_oos_predictions_",modeltype,finalfit_vintage,".Rds")))
