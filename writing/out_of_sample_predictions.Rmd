...
oos_data_calibration_applied<-oos_data_calibration_applied %>%
  select(c(year, stockarea, market_desc, semester, weighting, livlb, any_of(prob_names), .pred_class))
...
YRS_predictions<-oos_data_calibration_applied %>%
   group_by(year, stockarea,semester) %>%
   mutate(transactions=1) %>%
   summarise(across(c(starts_with(c("pred_"))), \(x) sum(x, na.rm = TRUE))) %>%
  ungroup() %>%
    select(year,semester, stockarea, starts_with("pred_"))
...
check1<-YRS_predictions %>%
  group_by(year,semester,stockarea, Market.Comb)%>%
  summarise(count=n()) %>%
  ungroup()%>%
...
YRS_predictions<-YRS_predictions %>%
  ...
  group_by(year,semester,stockarea, Market.Comb)%>%
  arrange(year,semester,stockarea,Market.Comb)
...
p_predictions <- ggplot(
    ambitious_YRS_predictions %>%filter(Market.Comb!="Unclassified"),
    aes(x = year_cont, y = live_metric_tons, fill = Market.Comb)
  ) +
  geom_col(
    position = position_stack(),
