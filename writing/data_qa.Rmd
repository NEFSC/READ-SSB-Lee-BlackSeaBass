...
<!--- RETIRED. This report read the A0*-stem .dta files produced by the
     Stata commercial processing chain (stata_code/.../commercial/A01-A04.do),
     which was retired. The live R chain writes .Rds only, so the read_dta()
     calls below can no longer be satisfied. Retained for reference; do not
     expect this to knit against a current data vintage. --->
...
keep_cols<-c("market_desc","dlrid","permit", "hullid","camsid","weighting", "mygear","price","priceR_CPI", "stockarea","state", "year","month", "semester","lndlb", "grade_desc", "trip_level_BSB")
...
# Persistence of market categories
Dealers with higher shares of Jumbos tend to have higher shares of Larges. Dealers with large shares of Unclassifieds have smaller shares of other things.

```{r dealer_classifications_pounds}
# Correlation of the Lag Shares of Pounds

dealer_pounds<-combined_dataset %>%
  group_by(dlrid,year) %>%
  slice(1) %>%
  ungroup() %>%
  select(starts_with("LagSharePounds"))

knitr::kable(cor(dealer_pounds, use="pairwise.complete.obs"), caption='Correlations of Dealer Shares of Pounds', format.args = list(big.mark = ","), digits=3, align=c("l",rep('r',times=4)))  
```

```{r correlations_of_pounds}

dealer_pounds %>%
  ggplot(aes(x=LagSharePoundsUnclassified, y=LagSharePoundsJumbo)) + 
  geom_jitter(width = 0.05, height = 0.05)


dealer_pounds %>%
  ggplot(aes(x=LagSharePoundsUnclassified, y=LagSharePoundsLarge)) + 
  geom_jitter(width = 0.05, height = 0.05)


dealer_pounds %>%
  ggplot(aes(x=LagSharePoundsUnclassified, y=LagSharePoundsMedium)) + 
  geom_jitter(width = 0.05, height = 0.05)


dealer_pounds %>%
  ggplot(aes(x=LagSharePoundsUnclassified, y=LagSharePoundsSmall)) + 
  geom_jitter(width = 0.05, height = 0.05)
...
Qualitatively the same thing when I remove the dealers than are almost all "unclassified"


```{r dealer_classifications_poundstrim}
# Correlation of the Lag Shares of Pounds

dealer_pounds<-combined_dataset %>%
  filter(Frac2014TUnclassified<=.85) %>%
  group_by(dlrid,year) %>%
  slice(1) %>%
  ungroup() %>%
  select(starts_with("LagSharePounds"))

knitr::kable(cor(dealer_pounds, use="pairwise.complete.obs"), caption='Correlations of Dealer Shares of Pounds. Trimmed.', format.args = list(big.mark = ","), digits=3, align=c("l",rep('r',times=4)))  

```



```{r correlations_of_poundsTrim}
...
Qualitatively the same thing for Transactions


```{r dealer_classifications_Trans}
# Correlation of the Lag Shares of Trans

dealer_pounds<-combined_dataset %>%
  group_by(dlrid,year) %>%
  slice(1) %>%
  ungroup() %>%
  select(starts_with("LagShareTrans"))

knitr::kable(cor(dealer_pounds, use="pairwise.complete.obs"), caption='Correlations of Dealer Shares of Transactions', format.args = list(big.mark = ","), digits=3, align=c("l",rep('r',times=4)))  

```



```{r correlations_of_trans}

dealer_pounds %>%
  ggplot(aes(x=LagShareTransUnclassified, y=LagShareTransJumbo)) + 
  geom_jitter(width = 0.05, height = 0.05)


dealer_pounds %>%
  ggplot(aes(x=LagShareTransUnclassified, y=LagShareTransLarge)) + 
  geom_jitter(width = 0.05, height = 0.05)


dealer_pounds %>%
  ggplot(aes(x=LagShareTransUnclassified, y=LagShareTransMedium)) + 
  geom_jitter(width = 0.05, height = 0.05)


dealer_pounds %>%
  ggplot(aes(x=LagShareTransUnclassified, y=LagShareTransSmall)) + 
  geom_jitter(width = 0.05, height = 0.05)


```

Trimmed dataset shows the same thing.

```{r correlations_of_transactions}

dealer_pounds2<-combined_dataset %>%
  filter(Frac2014TUnclassified<=.85) %>%
  group_by(dlrid,year) %>%
  slice(1) %>%
  ungroup() %>%
...
knitr::kable(cor(dealer_pounds2, use="pairwise.complete.obs"), caption='Correlations of Dealer Shares of Transactions. Trimmed', format.args = list(big.mark = ","), digits=3, align=c("l",rep('r',times=4)))  
```



```{r correlations_of_trans2}
