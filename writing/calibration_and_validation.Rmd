...
by_hand<-validation_preds %>%
  uncount(lndlb)

uw_by_hand <-  bind_rows(
  roc_auc(by_hand, truth = market_desc,
          starts_with(".pred_")),
  mn_log_loss(by_hand, truth = market_desc,
              starts_with(".pred_")),
  brier_class(by_hand, truth = market_desc,
              starts_with(".pred_"))
)


UWvalidation_metrics <-  bind_rows(
  roc_auc(validation_preds, truth = market_desc,
          starts_with(".pred_")),
  mn_log_loss(validation_preds, truth = market_desc,
              starts_with(".pred_")),
...
UWvalidation_metrics


```

The model is tuned to brier classification. The weighted metric has a lower brier class, although that may not mean anything. 



```{r PR_Curve}
# pull the probability names
prob_names<-colnames(validation_preds) 
...
# the weighted Curves will look more jagged (less smooth). that is because there are repeated observations that stack up, so things are less smooth
final_prWeighted<-validation_preds %>%
  pr_curve(market_desc,  any_of(prob_names), case_weights=weighting) %>%
  autoplot()
ggsave(here("results","ranger","final",glue("Weighted_pr_curve{modeltype}{finalfit_vintage}.png")), plot=final_prWeighted)
...
unc_val_split<-initial_split(uncounted_validation,
                             prop=0.25,
                             strata=market_desc)
uv2<-training(unc_val_split) 
...
```{r Wcalibration_models, eval=TRUE}
...
write_rds(calibrate_weighted_multinom, file=here("results","ranger","final",glue(
  "calibrate_uncounted_multinom_{modeltype}{finalfit_vintage}.Rds")))


```

```{r apply_Weighted_calibration, eval=TRUE}
...
# the difference between these metrics is the calibration loss (A little weird). But basically, the "goodness of fit metric" improves when I apply a calibration. the multinomial model is slightly better than iso and beta and also theoretically better.
ESPR_raw
ESPR_caliso
ESPR_calbeta
ESPR_calmulti

CalLoss<-ESPR_raw-ESPR_calmulti
...
```{r other_calibs}
# Apply the uncounted multinomial calibration.
# At the end of the day, we don't like this calibration either, so we're not saving it to a pdf anymore.
cal_gg <- validation_data_multicalib_applied %>%
  cal_plot_windowed(
    truth          = market_desc,
    estimate       = c(.pred_Jumbo, .pred_Large, .pred_Medium, .pred_Small),
    step_size      = 0.025,
...
# --- 2. Build publication-ready faceted calibration plot ---
multi_applied_window <- ggplot(cal_data,
                aes(x = predicted_midpoint, y = event_rate)) +
  # perfect calibration reference line
  geom_abline(slope = 1, intercept = 0,
              linetype = "dashed", colour = "grey50", linewidth = 0.4) +
...
ggsave(
  here("results", "ranger", "final",
       glue("cal_multivalidate_window{modeltype}{finalfit_vintage}.pdf")),
...
```{r beta}
#just a rough one
# we don't like the beta either, so we're not savig it to a pdef.
...
cal_weighted_multi
#ggsave(here("results","ranger","final",glue("cal_wmulti_window{modeltype}{finalfit_vintage}.png")), plot=cal_weighted_multi)

```


```{r ROC_Curve, eval=FALSE}
# The ROC curves don't depend on calibration.  It should move out of this file.
...
roc_data <- validation_data_multicalib_applied %>%
  roc_curve(
    truth = market_desc,
...
roc_dataUW <- validation_data_multicalib_applied %>%
  roc_curve(
    truth = market_desc,
    .pred_Jumbo, .pred_Large, .pred_Medium, .pred_Small
  )
...
auc_data <- validation_data_multicalib_applied %>%
  roc_auc(
    truth     = market_desc,
    .pred_Jumbo, .pred_Large, .pred_Medium, .pred_Small,
    case_weights=weighting,
...
## Predictions on the test set without calibration
...
# Apply weighted to the test dataset


```{r apply_weighted_calibration}
# apply the weighted calibraiton
# Plot the calibrated predictions
# need to pub-ready this.
test_data_Weighted_calibration_applied <-
  test_data %>%
  cal_apply(calibrate_weighted_multinom)
...
calibratedW<-cal_plot_windowed(test_data_Weighted_calibration_applied, truth = market_desc, step_size = 0.05,include_points=FALSE)

calibratedW

# Pub Ready Calibration
# Extract data from the ggplot internals
cal_data <- calibratedW$data
...
# ggsave(
#   here("results", "ranger", "final",
#        glue("calW_windowed_Wtesting{modeltype}{finalfit_vintage}.pdf")),
#   plot   = p_cal,
#   width  = 84,
#   height = 88,     # suits 4 panels in one row; increase to 130 for 2x2 layout
#   units  = "mm",
